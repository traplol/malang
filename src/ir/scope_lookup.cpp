...
Scope_Lookup::Scope_Lookup(Malang_IR *alloc)
    : m_alloc(alloc)
    , m_current_scope(new Scope{nullptr, alloc, true})
{}

void Scope_Lookup::push(bool can_see_parent_scope)
{
    auto scope = new Scope{m_current_scope, m_alloc, can_see_parent_scope};
    m_current_scope = scope;
}

void Scope_Lookup::pop()
{
    auto currents_parent = m_current_scope->m_parent;
    assert(currents_parent);
    delete m_current_scope;
    m_current_scope = currents_parent;
}
