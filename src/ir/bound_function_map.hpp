...
struct Bound_Function
{
    ...
    Bound_Function(Function_Type_Info *fn_type)
        : m_is_native(false)
        , m_is_defined(false)
        , m_fn_type(fn_type)
    {
    ...
    inline bool is_defined() const { return m_is_defined; }
    inline void define(IR_Label *code)
    {
        // Define once.
        assert(!is_defined());
        m_u.code = code;
...
struct Bound_Function_Map
{
    ...
    bool declare_method(Type_Info *to_type, const std::string &name, Function_Type_Info *fn_type);
