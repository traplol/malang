...
byte *Disassembler::dis1(byte *p, int offset, std::string &out)
{
    std::stringstream ss;
    auto ins = static_cast<Instruction>(fetch8(p));
    auto ins_str = to_string(ins);
    ss << setfill('0') << setw(8) << std::hex << (uint32_t)offset << "  ";
    ss << setfill(' ');
    switch (ins)
    {
